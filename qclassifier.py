...
class Quanvolutional_NN:
    def __init__(self, filterdim=3, train_size=1000, circuit_small="yes", resize=10):
        self.singleQ = {
            "X-rotation": "RX",
            "Y-rotation": "RY",
            "Z-rotation": "RZ",
        ...
        self.width = self.resize - self.filterdim + 1
        self.heigth = self.resize - self.filterdim + 1
        self.epochs = 20
        self.validationsize = 0.2
        self.batch_size = 16

        self.ndata_filtered = 0
        self.small_circuit = circuit_small
        self.name = "Test_" + str(self.train_size) + "_" + str(self.epochs)
        self.path = (
            "/Users/niccolo/Desktop/mnist/results/"
            + str(self.filterdim)
            + "x"
            + str(self.filterdim)
            + "_filter/Filters_"
    ...
    def initialize_data(self):
        ...
        new_size = [self.resize, self.resize]
        ...
        if self.filter == "yes":
            ...
            self.ndata_filtered = len(x_train)
    ...
    def quanvolutional_filter(self, image, quanv_filter):
        ...
        qconvolutional_output = np.zeros(
            (qconvolutional_output_width, qconvolutional_output_heigth)
        )
        ...
        if self.classes == 2:
            ...
            model.add(
                tf.keras.layers.Dense(
                    self.classes, activation="softmax", name="classifier"
                )
            )
    ...
    def hybrid_model(self, images):
        x = self.quanvolutional_layer_2(images)
    ...
    """
    Mentre la differenza tra classical_model e hybrid_model è che il primo è una CNN,
    mentre il secondo passa il dato da layer a layer permettendono in questo modo di fare sia la parte
    quantum che quella classica invece di affrontare le due cose a blocchi separati.
    """
    """
    La differenza tra il quanvolutional_layer e il quanvolutional_layer_2
    è che nel _2 passo le immagini che voglio far evolvere, mentre in quello originale 
    mi faceva la q-convolution dell'intero dataset.

    Adesso devo trovare il modo di far "vedere" alla loss i parametri di questi 5 circuiti
    (filtri), in modo che vengano ottimizzati.
    """

    def create_hybrid_model(self):
        inputs = tf.keras.layers.Input(
            shape=(self.ndata_filtered, self.width, self.heigth, self.nfilter)
        )
        output = self.hybrid_model(inputs)
        model = tf.keras.Model(inputs=inputs, outputs=output)
        return model

    def quanvolutional_layer_2(self, image):
        filters = self.selected_filters()
        new_image = []
        ...
        for j in range(self.nfilter):
            output = self.quanvolutional_filter(image, filters[j])
            image.append(output)

        image = tf.stack(new_image, axis=-1)
    ...
    """
    La differenza tra training_loop e training_loop_2 è che il secondo creo un modello ibrido mentre il primo 
    quello a blocchi
    """

    def training_loop_2(self):
